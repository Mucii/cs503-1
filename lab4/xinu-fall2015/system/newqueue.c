...
/*------------------------------------------------------------------------
 *  s_newqueue  -  Allocate and initialize a queue in the global queue table
 *------------------------------------------------------------------------
 */
qid16	s_newqueue(void)
{
	static qid16	nextqid=NPROC;	/* Next list in queuetab to use	*/
	qid16		q;		/* ID of allocated queue 	*/
	...
	sendtab[queuehead(q)].qnext = queuetail(q);
	sendtab[queuehead(q)].qprev = EMPTY;
	sendtab[queuetail(q)].qnext = EMPTY;
	sendtab[queuetail(q)].qprev = queuehead(q);
	return q;
}
